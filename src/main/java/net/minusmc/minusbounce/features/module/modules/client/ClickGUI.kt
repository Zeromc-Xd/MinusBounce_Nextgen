...
import net.minusmc.minusbounce.ui.client.clickgui.rise.RiseClickGui
...
object ClickGUI: Module() {
    val styleValue = ListValue("Style", arrayOf("Minus", "Rise"), "Minus")
    ...
    fun saveClickGuiValues() {
        MinusBounce.fileManager.saveConfig(MinusBounce.fileManager.valuesConfig)
    }

    fun changeClickGui() {
        RiseRenderUtilSafe.reset()
        mc.displayGuiScreen(if (styleValue.get().equals("rise", true)) RiseClickGui() else DropDownClickGui())
    }

    fun reloadOpenGui() {
        val screen = mc.currentScreen
        if (screen is RiseClickGui || screen is DropDownClickGui) changeClickGui()
    }

    @EventTarget(ignoreCondition = true)
    fun onWorld(event: WorldEvent) {
        RiseClickGui.resetSessionState()
    }
    ...
    private object RiseRenderUtilSafe {
        fun reset() {
            try {
                net.minusmc.minusbounce.ui.client.clickgui.rise.render.RiseRenderUtil.clearScaleTransform()
